...
export async function pullCommand(
    ...
    try {
        ...
        let pipelineType: 'text-generation' | 'feature-extraction' | 'text-to-speech' | 'automatic-speech-recognition' = 'text-generation';

        const modelLower = modelId.toLowerCase();
        if (modelLower.includes('embed') || modelLower.includes('minilm') || modelLower.includes('bge')) {
        ...
        await pipeline(pipelineType, modelId, {
            ...
            progress_callback: (progress: { status: string; progress?: number; file?: string }) => {
                if (progress.status === 'progress' && progress.progress !== undefined) {
                    ...
                    process.stdout.write(`\r   [${bar}] ${percent}% ${progress.file || ''}`);
        ...
        logger.error('Download failed', { modelId, error: (error as Error).message });
        process.exit(1);
    }
}
